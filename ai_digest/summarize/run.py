...
from .prompts import SYSTEM_SUMMARY, SYSTEM_REVIEW, build_summary_user, integrity_issues
...
def review_passed(review):
    return (isinstance(review, dict) and review.get("complete") is True
    ...
    with operation("summary.article", url=item.get("url"), retries=0, cache_hit=False,
        ...
        try:
            ...
            for attempt in range(2):
                ...
                issues = integrity_issues(result)
                log["retries"] = attempt
                if attempt == 0:
